...
export function loadCart() {
  ...
  const list_product_cart = document.querySelector(".list-product-cart");
  if (cart.length >= 1) {
    list_product_cart.innerHTML = "";
                ...
                <div class="cart-item">
                    ...
                    <div class="product-cart-details">
                        ...
                        <div class="product-price">
                            ${element.quantity} X <ins>${element.price}</ins>
                        </div>
                    </div>
                    <div class="product-cart-action">
                        <button class="product-action-button" onclick="removeFromCart(${element.id})">
    ...
    list_product_cart.innerHTML = "Nothing in here";
  }
...
function caculateSubTotal(cart) {
  ...
  if (cart.length >= 1) {
    cart.forEach((c) => {
      const price = c.price.split(/[.]/);
      total += parseFloat(price.join("").slice(2)) * c.quantity;
    });

    document.querySelector(".total-price-value").textContent = `Rp ${formatCash(
      String(total)
    )}`;
  } else {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("open-offcanvas-cart")
